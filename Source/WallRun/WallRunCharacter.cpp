...
void AWallRunCharacter::GetWallRunSideAndDirection(const FVector& Normal, EWallRunSide& OutSide, FVector& OutDirection) const
{
	if (FVector::DotProduct(Normal, GetActorRightVector()) > 0)
	{
		OutSide = EWallRunSide::Left;
		OutDirection = FVector::CrossProduct(Normal, FVector::UpVector).GetSafeNormal();
	}
	else
	{
		OutSide = EWallRunSide::Right;
		OutDirection = FVector::CrossProduct(FVector::UpVector, Normal).GetSafeNormal();
	}
}
...
{
	GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Red, TEXT("Wall run stoped"));
